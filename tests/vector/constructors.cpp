...
void	constructor(void) {
	{
		...
		display_output("vector.data(): ", iv_std.data(), iv_ft.data());
		display_output("vector.size(): ", iv_std.size(), iv_ft.size());
		iv_std.push_back(42);
		iv_ft.push_back(42);
		display_output("vector.push_back(42): ", iv_std[0], iv_ft[0]);
		display_output("vector.size(): ", iv_std.size(), iv_ft.size());

		display_header("Default constructor | vector<vector<int>>");
		std::vector<ft::vector<int>>	vv_std;
		ft::vector<ft::vector<int>>		vv_ft;

		display_output("vector.data(): ", vv_std.data(), vv_ft.data());
		display_output("vector.size(): ", vv_std.size(), vv_ft.size());
		vv_std.push_back(iv_ft);
		vv_ft.push_back(iv_ft);
		display_output("vector.push_back(vector<int>{42}): ", vv_std[0][0], vv_ft[0][0]);
		display_output("vector.size(): ", vv_std.size(), vv_ft.size());
		display_output("vector[0].size(): ", vv_std[0].size(), vv_ft[0].size());
		vv_std[0].push_back(4242);
		vv_ft[0].push_back(4242);
		display_output("vector[0].push_back(4242): ", vv_std[0][1], vv_ft[0][1]);
		display_output("vector[0].size(): ", vv_std[0].size(), vv_ft[0].size());
	}
	{
